...
func NewGoogleAuthHandler() *GoogleAuthHandler {
	...
	return &GoogleAuthHandler{config: configData}
}
...
func (h *GoogleAuthHandler) PostAuthToken(c *gin.Context) {
	...
	DB.Where("sso_id = ?", googleUser.ID).First(&userEstableshment)
	...
	if userEstableshment.ID == uuid.Nil {
		...
		if err := DB.Create(&userEstableshment).Error; err != nil {
	...
	tokenBuilder := core.NewTokenBuilder()
